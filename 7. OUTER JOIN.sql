...
SELECT
    *
FROM tb_emp E
LEFT JOIN tb_dept D -- OUTER는 생략이 가능함
ON E.dept_cd = D.dept_cd
;

-- Oracle JOIN
-- LEFT: JOIN조건 오른쪽에 (+)를 붙임
-- RIGHT: JOIN조건 왼쪽에 (+)를 붙임
-- FULL: 지원하지 않음
-- (+)가 붙으면 표준 (ANCI) 조인에서 ON절에 위치하는 조건
SELECT
    E.emp_nm,
    D.dept_nm
...
-- 사원이 존재하는 부서 14개
-- INNER JOIN의 결과 : 41행
-- LEFT OUTER JOIN의 결과 : 46행 (INNER 41행 + 부서없는사원5)
-- RIGHT OUTER JOIN의 결과 : 43행 (INNER 41행 + 사원없는부서2)
-- FULL OUTER JOIN의 결과 : 48행 (INNER 41행 + 5 + 2)
