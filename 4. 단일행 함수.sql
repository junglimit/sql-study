...
-- char타입 비교는 공백을 제외하고 문자가 같으면 같음
-- 그런데 *길이가 짧은쪽의 뒷공백을 넣고 비교*하기 때문에 앞공백이 들어간 경우엔 false
INSERT INTO char_compare VALUES ('101', 'SQLD', 'SQLD');
INSERT INTO char_compare VALUES ('102', 'SQLD', 'SQLA');
INSERT INTO char_compare VALUES ('103', 'SQLD', '  SQLD');-- false
...
-- varchar의 경우엔 글자 수가 다를때 따로 뒷공백을 넣거나 하지 않음.
-- 길이 비교에서 다르다면 바로 false
...
SELECT * FROM varchar_compare
WHERE sn = '101'
    AND char_4 = TRIM(varchar_6) -- 앞 뒤 공백제거
;
...
WHERE sn = '101'
    AND char_4 = 'SQLD  ' -- true , 공백이 있더라도 뒷공백을 추가하여 문자열만 비교함
;
...
FROM varchar_compare
WHERE sn = '101'
    AND varchar_6 = 'SQLD' -- false , 얜 걍 공백을 문자로 판단함
;
...
SELECT 
    CONCAT('SQL', 'Developer'),   -- 문자열을 결합 (||써서 붙이는건 oracle에서만 사용 가능)
...
SELECT
    NULLIF('A','A') -- 두개의 파라미터가 같으면 NULL값 출력
FROM dual;
...
SELECT 
    emp_no
    , emp_nm
    , NVL(direct_manager_emp_no, '최상위관리자') AS 관리자 -- 앞 파라미터가 null이면 두번째 파라미터로 변경
FROM tb_emp
;

SELECT 
    emp_nm,
     NVL(emp_nm, '존재안함') AS emp_nm, -- 조회자체가 안된 정보는 null이 아니라 변경X
     ADDR 
FROM tb_emp
WHERE emp_nm = '이정직2';
...
SELECT 
    -- emp_nm
    -- MAX(emp_nm) -- 없는 데이터의 MAX값을 찾으면 null값이 조회됨
    -- NVL(emp_nm, '존재안함') AS emp_nm
    NVL(MAX(emp_nm), '존재안함') AS emp_nm  -- 응용
FROM tb_emp
WHERE emp_nm = '이승엽';
