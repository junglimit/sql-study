...
CONNECT BY PRIOR A.emp_no = A.direct_manager_emp_no -- 자식 앞에 PRIOR(순방향)
...
SELECT
    emp_no, -- 자식
    emp_nm,
    direct_manager_emp_no -- 부모 (자식이 같은 값을 가질 수 있음)
FROM tb_emp
;
...
WHERE EXISTS (
        SELECT
            D.dept_cd -- EXISTS 에서는 SELECT가 의미가 없음
        FROM tb_emp E
        WHERE addr LIKE '%강남%' 
